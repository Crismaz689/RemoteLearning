...
public class EmailService : IEmailService
{
    ...
    public async Task SendCredentials(string username, string password, string recipientEmailAddress)
    {
        ...
        email.Body = new TextPart(TextFormat.Html) { Text = message.ToString()};
    ...
    private async Task SendEmail(MimeMessage message)
    {
        ...
        {
            await smtp.ConnectAsync(_settings.Smtp.Host, _settings.Smtp.Port, SecureSocketOptions.StartTls);
            await smtp.AuthenticateAsync(_settings.Smtp.Username, _settings.Smtp.Password);
